...
## from: https://github.com/wangleihitcs/DeepLab-V1-PyTorch/blob/master/nets/vgg.py

class VGG16_LargeFOV(nn.Module):
    def __init__(self, num_classes, input_size, split='train', init_weights=True):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(4, 64, kernel_size=3, stride=1, padding=1),  # changed: 3 -> 4 channels
            nn.ReLU(True),
            ...
            ### fc8
            nn.Conv2d(1024, num_classes, kernel_size=1, stride=1, padding=0)  # changed: output → 2
        )
    ...
    def _initialize_weights(self):
        for name, m in self.named_modules():
            if isinstance(m, nn.Conv2d):
                if name == 'features.38':  # final layer

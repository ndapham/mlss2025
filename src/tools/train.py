...
def train_epoch(cfg, train_state, model, data_loader, optimizer, device, criterion):
    ...
    return model, total_loss / len(data_loader), train_state
...
def eval_epoch(cfg, model, val_loader, device, criterion):
    ...
    return model, total_loss / len(val_loader)
...
def test_epoch(model, test_loader, criterion, device):
    ...
    for i, batch_infor in tqdm(enumerate(test_loader)):
        ...
        test_loss = criterion(batch_logits, batch_annos)
        total_loss += test_loss.item()
        
        batch_preds = (batch_logits > 0).to(batch_logits.dtype)
        preds.append(batch_preds)
        annos.append(batch_annos)
    
    total_pred = torch.cat(preds, dim=0).view(-1, H, W).detach().cpu().numpy()
    total_anno =  torch.cat(annos, dim=0).view(-1, H, W).detach().cpu().numpy()
    ...
    return  model, total_loss / len(test_loader), m_iou, fg_iou, bg_iou
...
def train(cfg, model_name):
    ...
    for epoch in range(cfg["training"]["epochs"]):
        ...
        model.train()
        model, mean_train_loss, train_state = train_epoch(cfg, train_state, model, train_loader, model_optimizer, device, criterion)
        wandb.log({
        ...
        model.eval()
        with torch.no_grad():
            model, mean_test_loss, m_iou_score, fg_score, bg_score = test_epoch(model, test_loader, criterion, device)
        wandb.log({
        ...
        if epoch % 30 == 1 or m_iou_score > best_val_iou:
            ...
            save_path = os.path.join(cfg["paths"]["checkpoints"], save_file_name,)
